...
std::string HOTP(std::string key, std::basic_string<unsigned char> data,
                 int bin_code_size) {
  if (data.size() != 8) {
  ...
  int bin_code =
      ((result.at(offset) & 0x7f) << 24 | (result.at(offset + 1) & 0xff) << 16 |
       (result.at(offset + 2) & 0xff) << 8 | (result.at(offset + 3) & 0xff));

  std::string bin_code_str = std::to_string(bin_code);

  return bin_code_str.substr(bin_code_str.size() - bin_code_size,
                             bin_code_size);
}
...
std::string TOTP(std::string key, uint64_t time, int bin_code_size,
                 long long interval) {
